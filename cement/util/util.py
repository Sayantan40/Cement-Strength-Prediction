...
def load_data(file_path: str, schema_file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        dataframe = dataframe.rename(columns={'fine_aggregate ':'fine_aggregate'})

        error_messgae = ""


        for column in  [i.replace(" ","") for i in list(dataframe.columns)]:  
            
            if column in [i.replace(" ","") for i in list(schema.keys())]:
